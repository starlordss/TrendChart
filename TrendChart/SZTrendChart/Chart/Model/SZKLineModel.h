...
@class SZTrendChartGroupModel;

typedef NS_ENUM(NSInteger, SZTrendChartCoinType) {
    SZTrendChartCoinTypeBTC = 1,   //比特币
    SZTrendChartCoinTypeETH,       //以太坊
    SZTrendChartCoinTypeNone       //未定义类型
};


@interface SZKLineModel : NSObject

#pragma 外部初始化

@property (nonatomic, assign) SZTrendChartCoinType coinType; //!< 货币类型
@property (nonatomic, strong) SZKLineModel *previousKlineModel; //!< 前一个Model
@property (nonatomic, strong) SZTrendChartGroupModel *parentGroupModel; //!< 父ModelArray:用来给当前Model索引到Parent数组
