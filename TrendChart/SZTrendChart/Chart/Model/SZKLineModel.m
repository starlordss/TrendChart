...
- (CGFloat)Volume_MA7 {
    if([SZTrendChartGlobalVariable isEMALine] == SZTrendChartTargetLineStatusMA) {
        if (!_Volume_MA7) {
...
- (SZTrendChartGroupModel *)parentGroupModel {
    if(!_parentGroupModel) {
        _parentGroupModel = [SZTrendChartGroupModel new];
    }
...
- (CGFloat)calcRSIWithDays:(NSInteger)daysCount {
    ...
    if (selfIndex < daysCount) {
        ...
        for (NSInteger i = selfIndex; i >= selfIndex - daysCount; i --) {
            SZKLineModel *model = self.parentGroupModel.models[i];
